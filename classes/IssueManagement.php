...
class IssueManagement
{
    ...
    public function addIssueFormFields(string $hookName, array $params): bool
    {
        ...
        $smarty->assign(['issuePreselectionIsOpen' => $isOpen, 'issuePreselectionEditors' => $assignedEditors, 'issuePreselectionEditorOptions' => $editorOptions,]);
    ...
    public function getEditorOptions(Context $context): array
    {
        ...
        $managers = Repo::user()->getCollector()->filterByContextIds([$contextId])->filterByRoleIds([Role::ROLE_ID_MANAGER])->getMany();

        foreach ($managers as $user) {
            $editorIds[$user->getId()] = $user->getFullName();
        }

        $subEditors = Repo::user()->getCollector()->filterByContextIds([$contextId])->filterByRoleIds([Role::ROLE_ID_SUB_EDITOR])->getMany();
