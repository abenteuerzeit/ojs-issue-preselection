    ...
    it("Shows issue selector in submission wizard", function () {
            ...
            if ($body.find('iframe[id*="abstract"]').length > 0) {
                ...
                cy.window().then((win) => {
                    if (win.tinymce && win.tinymce.editors) {
                        const abstractEditor = win.tinymce.editors.find((e) => e.id && e.id.includes("abstract"));
                        if (abstractEditor) {
                            abstractEditor.setContent("<p>This is a test abstract for the submission.</p>");
        ...
        cy.get("body").then(($body) => {
            if ($body.find('input[type="file"]').length > 0) {
                ...
                cy.get(".submissionWizard__footer button").contains("Continue").scrollIntoView().click({ force: true });
                cy.wait(2000);
            }
        });
        ...
        cy.get('input[type="radio"][name*="userGroupId"]').first().scrollIntoView().check({ force: true });

        cy.get(".pkpFormPage__footer button").contains("Save").scrollIntoView().click({ force: true });
        cy.wait(2000);
